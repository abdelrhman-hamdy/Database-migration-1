...
docs_number1=$(mongosh --quiet --host $mongodb -u $DB_USERNAME -p $DB_PASSWORD --eval 'use server' --eval 'db.customer.countDocuments({})')
...
docs_number2=$(mongosh --quiet --host $mongodb -u $DB_USERNAME -p $DB_PASSWORD --eval 'use server' --eval 'db.customer.countDocuments({})')
